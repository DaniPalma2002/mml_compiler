...
void mml::xml_writer::do_next_node(mml::next_node * const node, int lvl) {
  os() << std::string(lvl, ' ') << "<" << node->label() << ">" << node->val() << "</" << node->label() << ">" << std::endl;
}

void mml::xml_writer::do_return_node(mml::return_node * const node, int lvl) {
  //ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  if(node->expr()) node->expr()->accept(this, lvl + 4);
  closeTag(node, lvl);
}
...
void mml::xml_writer::do_null_ptr_node(mml::null_ptr_node * const node, int lvl) {
  openTag(node, lvl);
  closeTag(node, lvl);
}
